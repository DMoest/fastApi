...
class Settings(BaseSettings):
    ...
    pyenv_version: str = Field(default="3.11.4",
                               json_schema_extra={"env_name": "PYENV_VERSION"})
    app_debug: bool = Field(default=False,
                            json_schema_extra={"env_name": "APP_DEBUG"})

    # --- Application -------------------------------------------------------
    app_name: str = Field(default="Fast API application",
                          json_schema_extra={"env_name": "APP_NAME"})
    app_host: str = Field(default="localhost",
                          json_schema_extra={"env_name": "APP_HOST"})
    app_port: int = Field(default=1337,
                          json_schema_extra={"env_name": "APP_PORT"})
    app_reload: bool = Field(default=True,
                             json_schema_extra={"env_name": "APP_RELOAD"})

    # --- Secret Keys (JWT) --------------------------------------------------
    app_algorithm: str = Field(default="HS256",
                               json_schema_extra={"env_name": "APP_ALGORITHM"})
    app_jwt_secret_key: str = Field(default="a_secret_key", json_schema_extra={
        "env_name": "APP_JWT_SECRET_KEY"})

    # --- Logging settings ---------------------------------------------------
    app_log_level: str = Field(default="DEBUG",
                               json_schema_extra={"env_name": "APP_LOG_LEVEL"})
    app_log_name: str = Field(default="api_logger",
                              json_schema_extra={"env_name": "APP_LOG_NAME"})
    app_log_file_name: str = Field(default="fastapi_log", json_schema_extra={
        "env_name": "APP_LOG_FILE_NAME"})
    app_log_file_mode: str = Field(default="w", json_schema_extra={
        "env_name": "APP_LOG_FILE_MODE"})
    app_log_file_size: int = Field(default=1000000, json_schema_extra={
        "env_name": "APP_LOG_FILE_SIZE"})
    app_log_file_count: int = Field(default=3, json_schema_extra={
        "env_name": "APP_LOG_FILE_COUNT"})
    app_log_format: str = Field(
        default="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
        json_schema_extra={"env_name": "APP_LOG_FORMAT"})
    app_log_formatter: str = Field(default="defaut", json_schema_extra={
        "env_name": "APP_LOG_FORMATTER"})
    app_log_propagate: bool = Field(default=False, json_schema_extra={
        "env_name": "APP_LOG_PROPAGATE"})
    app_log_file_dir: str = Field(default="./../logs", json_schema_extra={
        "env_name": "APP_LOG_FILE_DIR"})
    ...
    pg_db_name: str = Field(default="the_db_name",
                            json_schema_extra={"env_name": "PG_DB_NAME"})
    pg_db_host: str = Field(default="the_db_host",
                            json_schema_extra={"env_name": "PG_DB_HOST"})
    pg_db_port: int = Field(default=5432,
